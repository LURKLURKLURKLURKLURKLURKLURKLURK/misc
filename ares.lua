		...
		local s, e = pcall(function()
		    ...
		    if onscreen and check then
		        ...
		        if esp.misc_layout['health'].enabled then
                    ...
                    array.healthbar.Color = esp.misc_layout['health'].lower:lerp(esp.misc_layout['health'].higher,character.Stats.Health.Base.Value  / character.Humanoid.MaxHealth);
                    array.healthbar.From = v2((position.X - 5), position.Y + size.Y)
                    array.healthbar.To = v2(array.healthbar.From.X, array.healthbar.From.Y - (character.Stats.Health.Base.Value  / character.Humanoid.MaxHealth) * size.Y)
