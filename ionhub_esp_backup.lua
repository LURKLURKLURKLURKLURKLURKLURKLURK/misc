...
local utility = {}; do
    ...
    esp.add = function(player)
        if player ~= localplayer then 
            esp.drawings[player] = {
                flag_box    = {};
                box         = utility_draw("Square", {Thickness = 1; ZIndex = 3; Filled = false; Color = esp_settings.box.color});
                ...
                snapline          = utility_draw("Line", {Thickness = 2});
                angle             = utility_draw("Line", {Thickness = 2});
                chameleon         = utility.instance("Highlight", {Parent = core_gui; DepthMode = Enum.HighlightDepthMode.AlwaysOnTop});
            };

            -- // Text
            for i,v in pairs(esp_settings.text_settings) do 
                esp.drawings[player][i] = utility_draw("Text", {Color = v.color; Size = esp_settings.font_size; Font = esp_settings.font; Outline = true; Center = true, ZIndex = 3});
                esp.drawings[player][i .. "_bold"] = utility_draw("Text", {Color = v.color; Size = esp_settings.font_size; Font = esp_settings.font; Center = true, ZIndex = 3});
            end;

            -- // Flag box
            for i = 1, 8 do 
                esp.drawings[player]["flag_box"][i] = utility_draw("Line", {Thickness = 1; ZIndex = 4; Visible = true});
            end;
        end;
    end;
    ...
    esp.remove = function(player)
        if esp.drawings[player] then
            for i,v in pairs(esp.drawings[player]) do
                if i == "chameleon" then
                    destroy(v);
                    continue;
                elseif i == "flag_box" then
    ...
    esp.get_tool = function(player, character)
        ...
        if character then
            local tool = game.GetService(game,'ReplicatedStorage').Players[character.Name].Status.GameplayVariables.EquippedTool --find_first_child_of_class(character, "Tool");
            if tool then
                return tostring(tool.Value);
            end;
        end;
        return "None";
    end;
...
utility_connection(renderstepped, function()
    ...
    for plr, data in pairs(esp_drawings) do
        ...
        if esp_settings.enabled and on_screen and meter_conversion < esp_settings.maximal_distance then
            if preciseness == 1 then
        ...
        health_text.Text           = tostring(current_health);
